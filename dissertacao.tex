\documentclass[
	12pt,		% Tamanho da fonte
	openright,	% capítulos começam em pág ímpar (insere página vazia caso preciso)
	twoside,	% para impressão em verso e anverso. Oposto a oneside
	a4paper,	% Tamanho do papel
	english,	% Idioma adicional para hifenização
	brazil		% O último idioma é o principal do documento
]{abntex2}

% Pacotes
\input{config/pacotes}

% Personalizações de capa e folha de rosto específicos para a Poli-USP
\input{config/aliases}

% Metadados
\input{config/metadados}

% Formatação
\input{config/formatacao}

% Compila somente determinados capítulos
%\includeonly{capitulos/5-ciclo-de-vida, capitulos/7-publicacoes, capitulos/8-cronograma, capitulos/9-conclusao}

%\includeonly{capitulos/0-pre-textual}
%\includeonly{capitulos/1-introducao}
%\includeonly{capitulos/2-metodologia}
%\includeonly{capitulos/3-fundamentos}
%\includeonly{capitulos/4-arquitetura}
%\includeonly{capitulos/5-ciclo-de-vida}
%\includeonly{capitulos/6-prova-de-conceito}
%\includeonly{capitulos/7-publicacoes}
%\includeonly{capitulos/8-cronograma}
%\includeonly{capitulos/9-conclusao}
%\includeonly{capitulos/10-referencias}

\begin{document}

% Elementos pré-textuais
\include{capitulos/0-pre-textual}

% Elementos textuais
\textual
\include{capitulos/1-introducao}
\include{capitulos/2-metodologia}
\include{capitulos/3-fundamentos}
\include{capitulos/4-arquitetura}
\include{capitulos/5-ciclo-de-vida}
%\include{capitulos/6-prova-de-conceito}
\include{capitulos/7-publicacoes}
\include{capitulos/8-cronograma}
\include{capitulos/9-conclusao}

% Elementos pós-textuais
\postextual
\include{capitulos/10-referencias}

\end{document}
